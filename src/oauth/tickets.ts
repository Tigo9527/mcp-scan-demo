 ...
 * 自包含加密票据（seal / open）。
 *
 * 为什么不用「存起来再查」：部署是多副本的，而现有存储层撑不住 ——
 * `persist.ts` 无锁无 CAS、`users/stats` 按 instanceId 分片、`settings.json` 每个进程只读一次。
 * 结果就是副本 A 发出的东西副本 B 查不到。本项目里唯一跨副本可靠的机制是**自包含签名**
 * （`github.ts` 的 `createState()` 就是这么做的），这里沿用同一思路，但改成**加密**，
